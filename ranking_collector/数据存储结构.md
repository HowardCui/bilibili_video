...
榜单中的每个视频对应一条 `ranking_items` 记录。默认每个分区快照保存 Top 10，因此一份完整快照通常包含 10 条记录。
...
如果一个视频只出现在本次榜单、没有出现在上一份榜单，就无法计算它的区间增量；当前实现会跳过该视频。如果前后两份 Top 10 完全不重合，快照仍会正常保存，但变化结果列表为空。

## 8. 示例数据结构

假设第 12 次采集成功保存了知识区 Top 10，数据库中的关系类似：
